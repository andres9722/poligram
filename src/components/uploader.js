              ...
              if(file.type.match('image.*')) {
                ...
                uploadTask.on('state_changed', data => {
                  ...
                  storageRef.child(file.name).getDownloadURL()
                    .then(url => {
                      //c(url)
                      output.insertAdjacentHTML('afterbegin', `${successMsg('Tu foto se ha subido')}
                      <img src="${url}">`)
...
return `
  <article class="uploader content-section u-hide">
    <h2 class="uploader-title">Sube tus fotos</h2>
    <form name="upload" id="upload">
      <input type="file" id="uploader" multiple>
      <label for="uploader">
        <i class="fa fa-cloud-upload" title="Subir fotos"></i>
      </label>
    </form>
    ${progressBar()}
  </article>
